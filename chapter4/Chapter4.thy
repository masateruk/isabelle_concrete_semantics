...
"set Tip = {}" |
"set (Node t1 a t2) = (set t1) Un {a} Un (set t2)"  

fun ord :: "int tree \<Rightarrow> bool"  where
"ord Tip = True" |
"ord (Node t1 n t2) = ((ALL i : set t1. i \<le> n) & (ALL i : set t2. n \<le> i))"
...
"ins n (Node t1 m t2) = 
    (if n = m then Node t1 m t2 
     else if n < m then Node (ins n t1) m t2 
     else Node t1 m (ins n t2))"
...
lemma ins_le:"(ALL n: set t. n \<le> m) & i < m \<Longrightarrow> ALL n: set (ins i t). n \<le> m"
apply(induction t)
apply(auto)
done  

lemma ins_ge:"(ALL n: set t. m \<le> n) & ~(i < m) & i \<noteq> m \<Longrightarrow> ALL n: set (ins i t). m \<le> n"
apply(induction t)
apply(auto)
done  
...
apply(induction t)
apply(auto simp add:ins_le)
apply(auto simp add:ins_ge)
done
